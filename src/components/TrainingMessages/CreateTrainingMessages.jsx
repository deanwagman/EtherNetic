...
import ValidateTrainingMessages from './ValidateTrainingMessages';
...
export default () => {
  ...
  const onSubmit = async (e) => {
    ...
    while (true) {
      ...
      try {
        ...
        viewTransition(() => {
            setTrainingMessages((curr) => [...curr, adaptedConversation]);
        });
  ...
  if (trainingMessages.length) {
    const onApprove = (id) => {
      viewTransition(() => {
        const newTrainingMessages = trainingMessages.filter(
      ...
      try {
        const response = fetch('/api/training-messages', {
          ...
          body: JSON.stringify({
            messages: trainingMessages.find(
              (conversation) => conversation.id === id,
            ).messages,
          }),
        });
    ...
    return (
      <Container>
        <ValidateTrainingMessages
          onApprove={onApprove}
          onDisapprove={onDisapprove}
          conversations={trainingMessages}
        />
      </Container>
    );
  }
